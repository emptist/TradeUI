...
public final class TradeStats {
    ...
    private func printTrade(_ result: TradeResult) {
        ...
        print("❌ \(direction) profit: \(result.profit) entry: \(result.entryPrice), exit: \(result.exitPrice), stopLoss: \(result.stopLossHit) confidence: \(result.confidence) info: \(result.patternInformation)")
    }
